    ...
    : nh("~")
{
	...
	nh.param("voxel_size", voxel_size, {1.0});

    cloud_path.insert(0, package_path);
    tf_path.insert(0, package_path);
    map_path.insert(0, package_path);

    vg.setLeafSize(voxel_size,voxel_size, voxel_size); // for global map visualization
...
void Integrator<T_p>::main()
{
    ...
    for (auto itr=transforms.begin(); itr!=transforms.end(); itr++)
    {
        ...
        *merge_cloud += *trans_cloud;
		...
		typename pcl::PointCloud<T_p>::Ptr vc_map(new pcl::PointCloud<T_p>());
		vg.setInputCloud(merge_cloud);
		vg.filter(*vc_map);
		merge_cloud = vc_map;
